...
public class ground_rs_double implements Runnable {
    ...
    private double calcVolume(double depth) {
        double opposite = depth - tanAlpha;
        if (alpha == 0) {
            return depth;
        }
        if (depth >= 1.0) {
            if (opposite >= 0.0) {
                return 1.0 - (1.0 - opposite) * (1.0 - opposite) / tanAlpha / 2.0;
            } else {
                return depth * depth / tanAlpha / 2.0 - (depth - 1.0) * (depth - 1.0) / tanAlpha / 2;
            }
        } else {
            if (opposite >= 0.0) {
                return (depth + opposite) / 2.0;
            } else {
                return depth * depth / tanAlpha / 2.0;
            }
        }
    }

    private void process(int x, int y, int d, double depth) {
        if (x < 0 || y < 0 || x >= hilbert.length || y >= hilbert.length || !hilbert[x][y]) {
            return;
        }
        depth = Math.min(depth, 1 + tanAlpha);
        if (depth < eps) {
            return;
        }
        volume += calcVolume(depth);
    ...
    public static void main(String[] args) {
        new Thread(new ground_rs_double()).start();
    }
}
